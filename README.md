...
- **C++**で**RGBD画像処理**を行い、**React**で**ユーザーインターフェース**を開発する、[身体障がい者向けジェスチャインタフェース](http://gesture-interface.jp)プロジェクト。**異言語間の連携**や**リアルタイム処理**を経験。
- **Unity（C#）**で作成する、[災害時救護活動の訓練・学習用のモバイルアプリ](http://www.disaster-medutainment.jp)プロジェクト。**ゲームエンジンを活用した実用的な開発**スキルを習得。
